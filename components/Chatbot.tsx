  ...
  function renderMessageText(text: string) {
    ...
    while ((m = r.exec(text)) !== null) {
      ...
      if (m[1] !== undefined) {
        ...
        if (linkMatch) {
          parts.push(<a key={`l-${m.index}`} href={linkMatch[2]} className="text-[var(--crimson)] hover:underline">{linkMatch[1]}</a>)
        } else {
          parts.push(m[0])
        }
      }
        ...
        <div className="mb-4 w-[420px] max-w-[calc(100vw-48px)] h-[520px] max-h-[calc(100dvh-120px)] max-[479px]:w-full max-[479px]:h-full max-[479px]:max-w-full max-[479px]:max-h-full max-[479px]:mb-0 max-[479px]:rounded-none max-[479px]:border-0 rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.4)] flex flex-col overflow-hidden border border-white/[0.08]" style={{ background: 'rgba(10,0,8,0.92)', backdropFilter: 'blur(20px) saturate(180%)', paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
          <div className="px-4 py-3 flex items-center justify-between flex-shrink-0" style={{ background: 'linear-gradient(135deg, var(--crimson), var(--violet))', paddingTop: 'calc(0.75rem + env(safe-area-inset-top, 0px))' }}>
            <div className="flex items-center gap-3">
              ...
              <div><p className="text-white text-sm font-bold leading-none" style={{ fontFamily: "'Syne', sans-serif" }}>TamilCinemaHub AI</p><div className="flex items-center gap-1.5 mt-0.5"><span className="w-1.5 h-1.5 bg-green-400 rounded-full" /><p className="text-white/70 text-xs">Online</p></div></div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3" style={{ background: 'rgba(15,2,20,0.6)', scrollbarWidth: 'thin' }}>
            {messages.map((msg, index) => (
              <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                {msg.role === 'system' ? (
                  <div className="w-full flex justify-center">
                    <div className="flex items-center gap-2 text-xs px-3.5 py-2.5 rounded-lg max-w-[85%]" style={{ background: 'rgba(240,180,41,0.1)', border: '1px solid rgba(240,180,41,0.25)', color: 'var(--gold)' }}>
                      <span>&#x23F1;</span>
                  ...
                  <>
                    {msg.role === 'assistant' && (<div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mr-2 mt-1" style={{ background: 'var(--crimson)' }}><span className="text-white text-xs">&#x1F3AC;</span></div>)}
                    <div className={`max-w-[78%] ${msg.role === 'user' ? 'items-end' : 'items-start'} flex flex-col`}>
                      <div className={`px-3.5 py-2.5 text-sm leading-relaxed whitespace-pre-line rounded-xl ${msg.role === 'user' ? 'text-white rounded-br-sm shadow-sm' : 'rounded-bl-sm'}`} style={msg.role === 'user' ? { background: 'linear-gradient(135deg, var(--crimson), var(--vermillion))' } : { background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.7)' }}>{msg.role === 'assistant' ? renderMessageText(msg.content) : msg.content}</div>
                      {msg.provider && <p className="text-[10px] mt-1 px-1" style={{ color: 'rgba(255,255,255,0.3)' }}>via {msg.provider}</p>}
                    </div>
                  </>
                )}
              </div>
            ))}
            {isLoading && !streamingText && (<div className="flex justify-start"><div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mr-2 mt-1" style={{ background: 'var(--crimson)' }}><span className="text-white text-xs">&#x1F3AC;</span></div><div className="rounded-xl rounded-bl-sm px-4 py-3 flex gap-2 items-center" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' }}><div className="flex gap-1.5"><span className="w-2 h-2 rounded-full animate-bounce" style={{ background: 'var(--crimson)', animationDelay: '0ms' }} /><span className="w-2 h-2 rounded-full animate-bounce" style={{ background: 'var(--crimson)', animationDelay: '150ms' }} /><span className="w-2 h-2 rounded-full animate-bounce" style={{ background: 'var(--crimson)', animationDelay: '300ms' }} /></div></div></div>)}
            {streamingText !== null && (<div className="flex justify-start"><div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mr-2 mt-1" style={{ background: 'var(--crimson)' }}><span className="text-white text-xs">&#x1F3AC;</span></div><div className="max-w-[78%] flex flex-col items-start"><div className="px-3.5 py-2.5 text-sm leading-relaxed whitespace-pre-line rounded-xl rounded-bl-sm" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.7)' }}>{renderMessageText(streamingText)}<span className="inline-block w-0.5 h-4 ml-0.5 align-text-bottom animate-pulse" style={{ background: 'var(--crimson)' }} /></div></div></div>)}
            {!isLoading && !streamingText && (() => { for (let i = messages.length - 1; i >= 0; i--) { const msg = messages[i]; if (msg.role === 'assistant' && msg.suggestions && msg.suggestions.length > 0) { const isLatest = !messages.slice(i + 1).some(m => m.role === 'assistant'); if (!isLatest) break; return (<div className="flex justify-start pl-8"><div className="flex flex-wrap gap-1.5 max-w-[78%]">{msg.suggestions.map((s) => (<button key={s} onClick={() => handleSuggestionClick(s)} className="text-[11px] px-3 py-1.5 rounded-full transition-all whitespace-nowrap active:scale-95" style={{ background: 'rgba(212,41,26,0.1)', border: '1px solid rgba(212,41,26,0.25)', color: 'var(--crimson)' }}>{s}</button>))}</div></div>) } } return null })()}
            <div ref={messagesEndRef} />
          </div>
          {messages.length === 1 && (<div className="px-3 py-2 flex gap-2 overflow-x-auto" style={{ background: 'rgba(15,2,20,0.4)', borderTop: '1px solid rgba(255,255,255,0.06)', scrollbarWidth: 'none' }}>{['Best 2026 movies', 'Vijay movies', 'Recommend thriller'].map((s) => (<button key={s} onClick={() => setInput(s)} className="flex-shrink-0 text-xs px-3 py-1.5 rounded-full transition-colors whitespace-nowrap" style={{ background: 'rgba(212,41,26,0.1)', border: '1px solid rgba(212,41,26,0.25)', color: 'var(--crimson)' }}>{s}</button>))}</div>)}
          <div className="px-3 py-3 flex gap-2 flex-shrink-0" style={{ background: 'rgba(10,0,8,0.8)', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
            <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder={isLoading || streamingText ? 'Generating...' : 'Ask about Tamil movies...'} disabled={isLoading || rateLimitCountdown > 0} className="flex-1 rounded-lg px-4 py-2 text-sm outline-none disabled:opacity-50 transition-all placeholder:text-white/35" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)', color: '#fff', fontFamily: "'DM Sans', sans-serif" }} />
            {(isLoading || streamingText) ? (<button onClick={stopGeneration} className="w-9 h-9 hover:opacity-90 text-white rounded-lg flex items-center justify-center flex-shrink-0 transition-all active:scale-95 animate-pulse" style={{ background: 'var(--crimson)' }}><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><rect x="4" y="4" width="16" height="16" rx="3" /></svg></button>) : (<button onClick={() => sendMessage()} disabled={rateLimitCountdown > 0 || !input.trim()} className="w-9 h-9 text-white rounded-lg flex items-center justify-center flex-shrink-0 transition-all active:scale-95 disabled:opacity-30" style={{ background: 'var(--crimson)' }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="22" y1="2" x2="11" y2="13" /><polygon points="22 2 15 22 11 13 2 9 22 2" /></svg></button>)}
          </div>
        </div>
      )}
      <button onClick={() => setIsOpen(!isOpen)} className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-300 active:scale-95 ${isOpen ? 'shadow-md max-[479px]:hidden' : 'text-white shadow-lg hover:scale-105'}`} style={!isOpen ? { background: 'linear-gradient(135deg, var(--crimson), var(--violet))', boxShadow: '0 8px 30px rgba(212,41,26,0.4)' } : { background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)' }} aria-label={isOpen ? 'Close chat' : 'Open TamilCinemaHub AI chat'}>
        {isOpen ? (<svg className="w-5 h-5" style={{ color: 'rgba(255,255,255,0.7)' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>) : (<svg className="w-6 h-6 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="7" width="20" height="15" rx="2" /><path d="M2 11h20M2 7l4-4M8 7l4-4M14 7l4-4M20 7l2-2M2 3h20" /></svg>)}
      </button>
    </div>
  )
}
