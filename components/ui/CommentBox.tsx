    ...
    <div className="mt-12 pt-8 border-t border-border">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-text-primary">Discussion</h2>
            <p className="text-xs text-text-muted">{totalComments} comment{totalComments !== 1 ? 's' : ''}</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="mb-8">
        <div className="p-5 rounded-2xl bg-bg-card border border-border">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex-1 flex gap-3">
              <input
                ...
                className="flex-1 px-3 py-2 rounded-xl bg-bg-elevated border border-border text-sm text-text-primary placeholder:text-text-muted/50 focus:outline-none focus:border-accent-gold/50 transition-colors"
                required
              />
              <input
                type="email"
          ...
          {/* Reply indicator */}
          {replyTo && (
            <div className="flex items-center gap-2 mb-3 px-3 py-2 rounded-lg bg-accent-gold/5 border border-accent-gold/20">
              <Reply size={12} className="text-accent-gold" />
              <span className="text-xs text-accent-gold">Replying to {replyAuthor}</span>
              <button type="button" onClick={() => { setReplyTo(null); setReplyAuthor('') }}
                className="ml-auto text-xs text-text-muted hover:text-text-primary">×</button>
            </div>
          )}
          ...
          <textarea
            ...
            rows={3}
            maxLength={1000}
            className="w-full px-3 py-2.5 rounded-xl bg-bg-elevated border border-border text-sm text-text-primary placeholder:text-text-muted/50 focus:outline-none focus:border-accent-gold/50 transition-colors resize-none mb-3"
            required
          />
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-accent-gold text-text-inverse text-sm font-semibold hover:bg-accent-gold-dim disabled:opacity-40 disabled:cursor-not-allowed transition-all"
            >
              {submitting ? (
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          <MessageCircle size={40} className="text-text-muted/30 mx-auto mb-3" />
        ...
        <div className="space-y-4">
          ...
          {hasMore && nextCursor && (
            <button
              ...
              className="w-full py-3 rounded-xl bg-bg-card border border-border text-sm text-text-muted hover:text-accent-gold hover:border-accent-gold/30 transition-all flex items-center justify-center gap-2"
            >
              <ChevronDown size={14} />
              Load more comments
            </button>
...
function CommentItem({
  ...
  const likeCount = (comment.likes?.length || 0) - (comment.likes?.includes('user') ? 0 : 0)

  return (
    <div className="group">
      {/* Main comment */}
      <div className={`p-4 rounded-2xl transition-all ${isEditing ? 'bg-accent-gold/5 border border-accent-gold/20' : 'bg-bg-card border border-border hover:border-border-light'}`}>
        <div className="flex gap-3">
          {/* Avatar */}
          <div className="w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold text-white shrink-0"
            ...
            {isEditing ? (
              <div className="space-y-2">
                <textarea
                  ...
                  rows={3}
                  maxLength={1000}
                  className="w-full px-3 py-2 rounded-xl bg-bg-elevated border border-border text-sm text-text-primary focus:outline-none focus:border-accent-gold/50 resize-none"
                />
                <div className="flex gap-2">
                  <button onClick={() => onSaveEdit(comment._key)}
                    className="px-3 py-1.5 rounded-lg bg-accent-gold text-text-inverse text-xs font-semibold hover:bg-accent-gold-dim transition-colors">
                    Save
                  </button>
                  <button onClick={onCancelEdit}
                    className="px-3 py-1.5 rounded-lg bg-bg-elevated border border-border text-xs text-text-muted hover:text-text-primary transition-colors">
                    Cancel
                  </button>
                </div>
              </div>
            ...
            {!isEditing && (
              <div className="flex items-center gap-4 mt-3">
                <button onClick={() => onLike(comment._key)}
                  className={`flex items-center gap-1.5 text-xs transition-colors ${isLiked ? 'text-red-400' : 'text-text-muted hover:text-red-400'}`}>
                ...
                <button onClick={() => onEdit(comment._key, comment.content)}
                  className="flex items-center gap-1.5 text-xs text-text-muted hover:text-accent-gold transition-colors opacity-0 group-hover:opacity-100">
                  <Edit3 size={12} />
                  Edit
                </button>
                <button onClick={() => onDelete(comment._key)}
                  className="flex items-center gap-1.5 text-xs text-text-muted hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100">
                  <Trash2 size={12} />
                  Delete
                </button>
              </div>
      ...
      {replies.length > 0 && (
        <div className="ml-8 mt-3 space-y-3">
          {replies.map(reply => (
            <div key={reply._key} className="p-4 rounded-2xl bg-bg-card/50 border border-border/50">
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-text-secondary leading-relaxed whitespace-pre-wrap">{reply.content}</p>
                  <div className="flex items-center gap-3 mt-2">
                    <button onClick={() => onLike(reply._key)}
                      className={`flex items-center gap-1 text-[11px] transition-colors ${(reply.likes?.includes('user')) ? 'text-red-400' : 'text-text-muted hover:text-red-400'}`}>
                      <Heart size={10} className={(reply.likes?.includes('user')) ? 'fill-current' : ''} />
                      {(reply.likes?.length || 0) > 0 && reply.likes!.length}
                    </button>
                    <button onClick={() => onDelete(reply._key)}
                      className="flex items-center gap-1 text-[11px] text-text-muted hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100">
                      <Trash2 size={10} />
                    </button>
                  </div>
                </div>
