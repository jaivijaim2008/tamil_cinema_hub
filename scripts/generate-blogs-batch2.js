#!/usr/bin/env node
/**
 * Generate remaining 5 blog posts (blogs 6-10 from original list)
 */
...
// Remaining 5 blogs (indices 5-9)
const TOPICS = [
  {
    title: "DC Release Date Confirmed: Lokesh Kanagaraj and Wamiqa Gabbi Next Big Thing",
    category: "News",
    ...
    tags: ["dc", "lokesh-kanagaraj", "2026"],
  },
  {
    title: "Mandaadi Release Date Locked: Soori Sports Action Drama Arrives This September",
    category: "News",
  ...
  {
    title: "Blast Box Office Update: Arjun Sarja Film Crosses 70 Crore Worldwide",
    category: "Box Office",
...
async function main() {
  console.log("Generating remaining 5 blog posts...\n");
  let ok = 0;

  for (let i = 0; i < TOPICS.length; i++) {
    const t = TOPICS[i];
    try {
      console.log(`[${i + 1}/5] ${t.title}`);
      const thumb = await genImg(t.imagePrompt, `thumb-rest-${i}-${Date.now()}.jpg`);

      const inlines = [];
      for (const pr of t.inlineImagePrompts) {
        const id = await genImg(pr, `inl-rest-${i}-${inlines.length}-${Date.now()}.jpg`, 1000, 562);
